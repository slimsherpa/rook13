...
class OracleBidder:
    ...
    def __init__(self, net, wp: WinProb, worlds: int = 16,
                 ...
                 oversample: int = 4, worlds_hot: int = 0):
        ...
        # adaptive depth: burn worlds where the winner's curse lives (high
        # floors); skim where a bid is a cheap option
        self.worlds_hot = worlds_hot
    ...
    def bid(self, g: Game, me: int, cands: list[int]) -> int:
        ...
        floor0 = g.min_next_bid()
        k = (self.worlds_hot if self.worlds_hot and floor0 is not None
             and floor0 >= 90 else self.worlds)
        worlds = sample_worlds(g, me, k, self.listen, rng,
...
def main():
    ...
    ap.add_argument("--worlds-hot", type=int, default=0,
                    help="world count for contested decisions (floor >= 90)")
    ...
    with ctx.Pool(args.workers, initializer=_init_worker,
                  initargs=(args.net, args.worlds, listen,
                            ...
                            args.oversample, args.worlds_hot)) as pool:
