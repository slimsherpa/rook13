...
nohup nice -n 5 $PY -m alpharook.widowtwin \
  ...
  --belief models/gen15.pt --widow-budget 10 \
  --proposer models/widowprop-v0.pt --label-rows \
  --seed-base $(( 16000000000 + B * 100000000 )) \
  --out runs/p13/gate10 --run-tag box${B} \
  >> runs/p13/gate10_box${B}.log 2>&1 &
